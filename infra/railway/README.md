...
| Symptom                                                                                | Fix                                                                                                                                                                                                                                                                                  |
| -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `The executable pnpm could not be found` / admin **502 Application failed to respond** | Admin must use Dockerfile + Start Command `node server.mjs` (Node static server binds `$PORT`). Clear any `pnpm`/`nginx` Custom Start Command. Set `RAILWAY_DOCKERFILE_PATH=apps/admin/Dockerfile`. Redeploy latest `apps/admin/Dockerfile`. Verify: `curl https://…/health` → `ok`. |
