...
| Symptom                                                                                | Fix                                                                                                                                                                                                                                                                                                                                                                                 |
| -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `The executable pnpm could not be found` / admin **502 Application failed to respond** | (1) Dockerfile + blank Start Command (or `node /app/server.mjs`). `RAILWAY_DOCKERFILE_PATH=apps/admin/Dockerfile`. (2) **Networking → public domain → Target port: leave empty / automatic** — do **not** hardcode `8080`/`80`/`3000` (mismatch with Railway `$PORT` causes 502 with a green build). (3) Deploy logs should show `retain-admin listening` and `PORT env: <number>`. |
