...
| Symptom                                                     | Fix                                                                                                                                                                                                                                                                                                                                   |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `The executable pnpm could not be found` (Create container) | Production images have **no pnpm**. Clear any Custom **Start Command** / **Build Command** that uses `pnpm`. API/worker start: `node dist/index.js`. Admin/portal: leave empty (nginx Dockerfile CMD) or `/docker-entrypoint.sh nginx -g 'daemon off;'`. Confirm `RAILWAY_DOCKERFILE_PATH` (e.g. `apps/api/Dockerfile`) and redeploy. |
