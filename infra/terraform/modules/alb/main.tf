resource "aws_security_group" "alb" {
  name        = "retain-${var.environment}-alb"
...
resource "aws_lb" "this" {
  ...
  tags = { Name = "retain-${var.environment}-alb" }
}
...
resource "aws_lb_listener_rule" "ai" {
  ...
  condition {
    host_header { values = [var.ai_domain] }
  }
}

resource "aws_lb_listener_rule" "worker_metrics" {
  listener_arn = aws_lb_listener.https.arn
  priority     = 20
...
resource "aws_lb_target_group" "api" {
  name        = "retain-${var.environment}-api"
  port        = 3001
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
...
resource "aws_lb_target_group" "ai" {
  name        = "retain-${var.environment}-ai"
  port        = 8000
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
...
resource "aws_lb_target_group" "worker" {
  name        = "retain-${var.environment}-worker"
  port        = 3002
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
